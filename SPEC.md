...
| Действие | Строка |
|---|---|
| Roll | `Roll {count} → {результат}` |
| ReRoll | `Reroll {count} {что было} → {результат}`; без «что было» — `Reroll {count} → {результат}` |
| Add | `Add {count}` |
| Delete | `Remove {count}` |
| Move | `Move {count} → {value}` |
| Clear | `Clear` |

Roll и ReRoll сохраняют, сколько кубиков было брошено, и результат — сгруппированные значения брошенных кубиков по убыванию: `6×2, 5, 3` (одиночное значение без счётчика). «Что было» у ReRoll — значения перебрасываемых кубиков до действия в нотации выбора (например, `6+`).
